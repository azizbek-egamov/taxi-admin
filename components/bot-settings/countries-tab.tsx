  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCountry) {
        ...
        await apiClient.createCountry(data as any)
      ...
      <CardContent>
        {message && (
          <Alert className={`mb-4 ${message.type === "success" ? "border-green-200 bg-green-50" : "border-red-200 bg-red-50"}`}>
            {message.type === "success" ? <CheckCircle className="h-4 w-4 mr-2 text-green-600" /> : <AlertCircle className="h-4 w-4 mr-2 text-red-600" />}
        ...
        <div className="flex justify-end mb-4">
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            <DialogTrigger asChild>
              <Button onClick={resetForm} className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700">
            ...
            <DialogContent className="sm:max-w-[600px]">
              ...
              <form onSubmit={handleSubmit} className="grid gap-4 py-4">
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Input id="nameUz" value={nameUz} onChange={(e) => setNameUz(e.target.value)} className="col-span-3" required />
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="nameRu" className="text-right">
                    Nomi (Rus)
                  </Label>
                  <Input id="nameRu" value={nameRu} onChange={(e) => setNameRu(e.target.value)} className="col-span-3" required />
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="nameCy" className="text-right">
                    Nomi (Kiril)
                  </Label>
                  <Input id="nameCy" value={nameCy} onChange={(e) => setNameCy(e.target.value)} className="col-span-3" />
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="nameTj" className="text-right">
                    Nomi (Tojik)
                  </Label>
                  <Input id="nameTj" value={nameTj} onChange={(e) => setNameTj(e.target.value)} className="col-span-3" />
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="nameKz" className="text-right">
                    Nomi (Qozoq)
                  </Label>
                  <Input id="nameKz" value={nameKz} onChange={(e) => setNameKz(e.target.value)} className="col-span-3" />
                </div>
                <DialogFooter>
                  <Button type="submit" disabled={isSaving}>{isSaving ? "Saqlanmoqda..." : "Saqlash"}</Button>
                </DialogFooter>
              </form>
            </DialogContent>
          </Dialog>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Nomi (Kiril)</TableHead>
              ...
              <TableBody>
                {countries.map((country) => (
                  <TableRow key={country.id}>
                    ...
                    <TableCell>{country.name_cy}</TableCell>
